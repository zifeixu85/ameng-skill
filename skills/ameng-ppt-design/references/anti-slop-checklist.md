...
- [ ] **P0 溢出（已有自动闸门，别只靠肉眼）**：先跑 **`node scripts/check-overflow.mjs <deck.html>`** —— headless 量每页每元素 vs 安全区（含 nowrap ink 溢出），**越界 exit 1 必须先修**。在浏览器里按 **`G`** 看红色越界边界+⚠角标定位。结构上：密集页用 `.slide__safe`（子项压缩不顶出），会变长的单行大标题/数字用 `.fit-text`（自动缩字）。装不下就拆两页。PNG 复看时仍重点查最后一行/最后一卡是否被裁。
...
先跑 `node scripts/check-overflow.mjs <deck.html>`（几何闸门，越界 exit 1 → 先修），再跑 `./scripts/render.sh <deck.html>` 出逐页 PNG，**对每一页**逐条问：
