...
|--------|--------|
| 点任意文字**就地修改**（含**讲者备注**，S 放映查看） | **PDF**（每页整图合成，jsPDF） |
| 点「**完成**」对**改动过的页**各存一版（刷新仍在） | **PPTX · 图片版**（每页满版整图，高保真） |
| **历史只看当前页** + **diff 旧→新** + 角标显示版本数 | **PPTX · 可编辑**（文字版，可在 PPT 改字，样式简化） |
...
> 键盘：`1–9`/`0` 跳页 · `F` 全屏 · `S` 备注 · `O` 总览 · `T` 明暗。
