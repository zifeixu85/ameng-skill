...
### ✎ 编辑 → 完成（自动存版本）
- 点「编辑」进入编辑模式 → 标题/正文/标签等文字出现虚线框，**点进去直接改**；**讲者备注**也会在底部以可编辑面板出现（放映时按 `S` 查看）。
- 点「**完成**」：退出编辑 + **对「本次有改动的页」各自动存一个版本**（localStorage，按文件路径区分；下次打开自动还原）。`Esc` 同效。
- 「**历史**」：只列**当前页**的版本，**显示与上一版的文字差异（旧 → 新）**，便于判断恢复哪一版；可**恢复**或删除。「历史」按钮上有**角标**显示当前页有几个版本（没改过就不显示）。
...
- **PPTX · 可编辑（文字版）**：把每页文字摆成可在 PowerPoint/WPS 里改的文本框（含角标/小标题），**样式简化**（纯底色 + 文本，丢 grain/高亮带等）。要保真用「图片版」，要改字用这版。
...
## 键盘速查
```
← → Space PgUp PgDn Home End  翻页   1–9 / 0  跳到第 N 页（0=第 10 页）
F 全屏放映（自动隐藏所有操作 UI）  S 备注  O 总览  T 浅色/深色
