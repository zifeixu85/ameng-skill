...
> dock 默认只是个低调 `⋯`；**鼠标移到屏幕最上面一条**或 hover 它即展开（移开自动收起）。

### ✎ 编辑 → 完成（自动存版本）
- 点「编辑」进入编辑模式 → 标题/正文/标签等文字出现虚线框，**点进去直接改**；**讲者备注**也会在底部以可编辑面板出现（放映时按 `S` 查看）。**备注可改、会保留，但不计入版本**（改备注不会产生新版本）。
- 点「**完成**」：退出编辑 + **对「本次正文有改动的页」各自动存一个版本**（localStorage，按文件路径区分；下次打开自动还原）。`Esc` 同效。
- 「**历史**」：只列**当前页**的版本，**最早一条是「默认版本（初始）」= 版本 1**，往上是每次改动；每条**显示与上一版的文字差异（旧 → 新）**，可**恢复**（恢复内容时保留你当前的备注）或删除。「历史」按钮**仅当本页有 ≥2 个版本（即真改过）时**显示数字角标。
...
- **PPTX · 可编辑（保真版）**：把每页**装饰（背景/高亮带/终端/截图框/grain）渲染成底图**，再在上面叠**可编辑文本框**——样式尽量还原、文字仍能在 PPT 里改（无重影）。字体会被 PowerPoint 替换、个别带装饰的文字位置可能略偏；要 100% 保真用「图片版」，要改字用这版。
