...
- **PPTX · 可编辑（分层版）**：每页拆 **3 层** —— ①**背景图**（页底色/分段色/grain）②**装饰图**（终端/截图框/图表/高亮带，**文字已抽走**，透明底）③**可编辑文字框**。所以**文字一律不切图、可在 PPT 里改**，装饰尽量还原（字体会被 PowerPoint 替换、个别位置可能略偏）。要 100% 保真用「图片版」，要改字用这版。
